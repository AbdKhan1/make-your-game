...
export function createBall(x, y) {
    let ball = document.createElement('div'),
        ...
        ballStartPosition = [
            gameView.offsetWidth / 2 - ballSettings.size / 2,
            70
        ];
...
export function BallMovement() {
    ...
    for (let i = 0; i < balls.length; i++) {
        let xDirection = ballsDirection[i][0],
            yDirection = ballsDirection[i][1];
...
function calculatePaddleBounce(ball) {
    ...
    switch (true) {
        ...
        case ball.x >= paddle.x &&
            ball.x <= paddle.x + paddleWidthSplit &&
            ball.y <= paddle.y:
            return [-bspeed, bspeed];

        //if the ball hits the second quarter of the paddle
        case ball.x > paddle.x + paddleWidthSplit &&
            ball.x <= paddle.x + 2 * paddleWidthSplit:
            return [-Math.ceil(bspeed / 2), bspeed];

        //if the ball hits the third quarter of the paddle
        case ball.x > paddle.x + 2 * paddleWidthSplit &&
            ball.x <= paddle.x + 3 * paddleWidthSplit:
            return [Math.ceil(bspeed / 2), bspeed];

        //if the ball hits the fourth quarter of the paddle
        case ball.x > paddle.x + 3 * paddleWidthSplit &&
        ...
        case ball.x > paddle.x + 3 * paddleWidthSplit &&
            ball.x <= paddle.x + 4 * paddleWidthSplit &&
            ball.y > paddle.y &&
