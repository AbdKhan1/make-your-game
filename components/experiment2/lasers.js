  ...
  for (let i = 0; i < aliens.length; i++) {
    if (i === random) {
      ...
      laserStartingPosition[0] =
        ...
        alien.getBoundingClientRect().width / 2 -
        laserSettings.width / 2;
      laserStartingPosition[1] =
        alien.getBoundingClientRect().top +
        alien.getBoundingClientRect().height;
...
export function laserMovement() {
  ...
  for (let i = 0; i < laserPositions.length; i++) {
    ...
    if (
      laserPositions[i][1] >=
      gameViewSettings.gameViewHeight -
        (laserSettings.height - gameViewSettings.borderWidth)
    ) {
      removeLaser(i);
    }
  }
