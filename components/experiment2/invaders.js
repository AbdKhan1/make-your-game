...
export function createAliens(level) {
    ...
    let aliensStartingY =
        ...
        (levels[level].bricks.numberOfRows *
    ...
    for (let r = 1; r <= levels[level].aliens.numberOfRows; r++) {
        for (let a = 1; a <= levels[level].aliens.numberOfAliens; a++) {
            ...
            alienPositions.push([
                alienStartingPosition[0],
                alienStartingPosition[1]
            ]);
            alienStartingPosition[0] +=
                invaderSettings.width + spaceBetweenAliens;
        }
...
export function alienMovement(level) {
    ...
    if (
        parseInt(aliencloseToBorder[1].style.left) <= 10 ||
        parseInt(aliencloseToBorder[0].style.left) >=
            gameView.offsetWidth - invaderSettings.width - 10
    ) {
    ...
    for (let i = 0; i < invaders.length; i++) {
        ...
        if (
            alienPositions[i][1] >=
            gameView.offsetHeight - 50 - invaderSettings.height
        ) {
            //gameover
            alienPositions[i][1] =
                levels[level].bricks.gapFromTop +
                (levels[level].bricks.numberOfRows *
                    (levels[level].bricks.brickHeight +
